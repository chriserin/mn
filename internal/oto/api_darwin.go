...
// _SMPTETime and _AudioTimeStamp mirror CoreAudioTypes.h's SMPTETime and
// AudioTimeStamp byte-for-byte (field order and natural alignment both
// match what the C compiler produces, so no manual padding is needed).
// Previously this package only ever passed a nil *_AudioTimeStamp (to
// AudioQueueStart, which doesn't dereference it unless doing scheduled
// starts), so a uintptr stub was enough; AudioQueueGetCurrentTime actually
// fills one in, so the real layout matters now.
